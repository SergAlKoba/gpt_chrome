...
async function login(email, password) {
  ...
  let response = await fetch("https://gotgood.ai/api/user/login/", requestOptions).catch((error) =>
  ...
  if (result?.auth_token || localStorage.getItem("token")) {
    ...
    let er =
      (result?.email ? result.email.join(", ") : null) ??
      (result?.password ? result.password.join(", ") : null) ??
      (result?.non_field_errors ? result.non_field_errors.join(", ") : null) ??
      result?.info ??
      result?.error;
...
async function register(email, password, username) {
  ...
  let response = await fetch("https://gotgood.ai/api/user/register/", requestOptions).catch((error) =>
    console.log("error", error)
  );
  let result = await response.json();
...
function createPopup(type) {
  ...
  signGoogleLink.setAttribute(
    "data-client_id",
    "855810422972-rnj3fvs55se2fbklll064ha30pp4fuca.apps.googleusercontent.com"
  );
